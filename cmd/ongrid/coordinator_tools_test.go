...
func TestCoordinatorRosterDerivesRegisteredCoreTools(t *testing.T) {
	registered := []aiopstoolsbase.BaseTool{
		...
		coordinatorToolStub{name: "query_systems"},
		coordinatorToolStub{name: "list_database_sources"},
		coordinatorToolStub{name: "analyze_database_status"},
		coordinatorToolStub{name: "list_metric_catalog"},
		coordinatorToolStub{name: "host_find_large_files"},
	}
	got := buildCoordinatorToolNames(registered)
	for _, want := range []string{
		"query_devices", "query_traceql", "query_knowledge", "read_source", "query_promql",
		"query_systems", "list_database_sources", "analyze_database_status", "list_metric_catalog",
	} {
