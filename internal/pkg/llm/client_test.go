...
func TestSamplingErrorTriggersRetry(t *testing.T) {
	...
	// "sol-max" is not matched by isReasoningModel; send an explicit
	// temperature so the reactive strip+retry path is exercised.
	_, err := client.Chat(context.Background(), ChatReq{
		Model:    "sol-max",
		Messages: []Message{{Role: "user", Content: "hi"}},
		Sampling: SamplingParams{Temperature: PtrFloat32(0.1)},
	})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
